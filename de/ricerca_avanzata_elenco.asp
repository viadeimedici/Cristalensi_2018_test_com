...
if cat>0 then
Set cs=Server.CreateObject("ADODB.Recordset")
sql = "SELECT Categorie1.PkId as PkId_1, Categorie1.Titolo_DE as Titolo_1, Categorie2.PkId as PkId_2, Categorie2.Titolo_DE as Titolo_2 "
...
sql = sql + "ORDER BY Categorie1.Titolo_DE ASC, Categorie2.Titolo_DE ASC"
cs.Open sql, conn, 1, 1
if cs.recordcount>0 then
...
<head>
    ...
    <style type="text/css">
		...
		<%if p>0 then%>
		<link rel="canonical" href="https://www.cristalensi.com/de/ricerca_avanzata_elenco.asp?cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=<%=order%>"/>
		<%end if%>
    ...
    <div class="container content">
        ...
        <div class="col-xl-12">
            <ol class="breadcrumb">
                ...
                <li class="active">Detaillierte Suche von Beleuchtungsartikeln</li>
            </ol>

						<%if titolo<>"" or prezzo_da<>0 or prezzo_a<>0 or title<>"" or produttore<>"" then%>
						<h1 class="title">
						Detaillierte Suche von Beleuchtungsartikeln
						 - Du hast gesucht: <%if titolo<>"" then%> <%=titolo%><%end if%><%if prezzo_da<>0 or prezzo_a<>0 then%> - Preis von <%=prezzo_da%> bis <%=prezzo_a%><%end if%>
						<%if title<>"" then%><br />Kategorie: <%=title%><%end if%><%if produttore<>"" then%><br />Marke: <%=produttore%><%end if%>
						</h1>
						<%end if%>
        ...
        <div class="col-md-3 hidden-xs">
            <div class=" top-buffer">
				...
				<%
					...
					if titolo<>"" then
						sql = sql + "AND (CodiceArticolo LIKE '%"&titolo&"%' OR CodiceArticolo_Azienda LIKE '%"&titolo&"%' OR Titolo_de LIKE '%"&titolo&"%') "
					end if
        ...
        <div class="col-md-9">
                        ...
                        <div class="container-fluid">
                            ...
                            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                                ...
                                <ul class="nav navbar-nav">
                                    <li <%if order=3 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="https://www.cristalensi.com/de/ricerca_avanzata_elenco.asp?cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=3"><i class="glyphicon glyphicon-eur"></i> - </a></li>
                                    <li <%if order=4 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="https://www.cristalensi.com/de/ricerca_avanzata_elenco.asp?cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=4"><i class="glyphicon glyphicon-eur"></i> + </a></li>
                                </ul>
                                <p class="navbar-text">alphabetisch</p>
                                <ul class="nav navbar-nav">
                                    <li <%if order=1 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="https://www.cristalensi.com/de/ricerca_avanzata_elenco.asp?cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=1">A/Z</a></li>
                                    <li <%if order=2 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="https://www.cristalensi.com/de/ricerca_avanzata_elenco.asp?cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=2">Z/A</a></li>
                                </ul>
                            </div>
                        </div>
                    </nav>
            ...
            <div class="row top-buffer">
						...
						Zur Zeit sind auf unserer Internetseite keine Produkte, die Ihren Suchkriterien entsprechen, vorhanden. Cristalensi hat aber mit vielen Firmen, die Beleuchtungsartikel herstellen, gute Kontakte. Wenn Sie ein bestimmtes Produkt kennen und dafür gerne einen Kostenvoranschlag hätten, füllen Sie bitte das Formular mit dem Namen des Produktes und weiteren Informationen aus. Wir werden Sie baldmöglichst kontaktieren: unsere Mitarbeiter stehen Ihnen für sämtliche Fragen zur Verfügung.<br /><a href="/de/contatti.asp">KONTAKTE!</a>
						</div>
				</div>
				<%
				end if
			...
			<div class="col-xl-12">
					...
					<div class="col-lg-6">
							<div class="form-group">
									<label for="titolo" class="col-sm-4 control-label">Produktname oder Code</label>
									<div class="col-sm-8">
											<input type="text" class="form-control" id="testo_ricerca" name="testo_ricerca">
									</div>
							</div>
							<div class="form-group">
									<label for="FkProduttore" class="col-sm-4 control-label">Marke</label>
									<div class="col-sm-8">
											<%
											Set cs=Server.CreateObject("ADODB.Recordset")
											...
											<option value="0">Wählen Sie einen Hersteller</option>
											<%
											Do While Not cs.EOF
											%>
					...
					<div class="col-lg-6">
							<div class="form-group">
									<label for="inputEmail3" class="col-sm-4 control-label">Mindestpreis</label>
									<div class="col-sm-8">
											<input type="text" class="form-control" id="prezzo_da" name="prezzo_da">
									</div>
							</div>
							<div class="form-group">
									<label for="prezzo_a" class="col-sm-4 control-label">Maximaler Preis</label>
									<div class="col-sm-8">
							...
							<div class="form-group">
									<div class="col-sm-offset-4 col-sm-8">
											<button type="submit" class="btn btn-danger">Suche Starten</button>
									</div>
							</div>
					</div>
					</form>
