...
<head>
    ...
    <meta name="description" content="<%if Len(Description)>0 then%><%=Description%><%else%>From Cristalensi you find a wide range of <%=Titolo_1%>: online discounts on all lighting products<%end if%>">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta property="og:description" content="<%if Len(Description)>0 then%><%=Description%><%else%>From Cristalensi you find a wide range of <%=Titolo_1%>: online discounts on all lighting products<%end if%>">
    ...
    <div class="container content">
        ...
        <div class="col-md-9">
            <div class="row top-buffer">
                ...
                sql = "SELECT * FROM Prodotti WHERE ((FkNewStile="&pkid_stile&") and (Offerta=0 or Offerta=2) and (Len(Titolo_EN)>0)) ORDER BY "&ordine&""
