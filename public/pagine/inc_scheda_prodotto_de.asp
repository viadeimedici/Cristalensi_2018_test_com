...
if id>0 then
	...
	if prod_rs.recordcount>0 then
		...
		Descrizione_prodotto=prod_rs("Descrizione_de")
...
<head>
    ...
    <meta property="og:description" content="<%=Titolo_prodotto%>-Produktdetails, <%=produttore%>, code <%=codicearticolo%>. Cristalensi Online-Shop von <%=Titolo_1_cat%>.">
    ...
    <div class="container content">
		...
		<div class=" clearfix" style="margin-top: 20px;">
			<div class="col-md-10 col-md-push-2">
                ...
                <li itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a href="/lichtmarkenlampen-innenbeleuchtung-aussenbeleuchtung/<%=Url_gr%>" itemprop="item" title="<%=Titolo_1_gr%>"><span itemprop="name"><%=Titolo_1_gr%></span></a><meta itemprop="position" content="2" /></li>
                <li itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a href="/lichtmarkenlampen-innenbeleuchtung-aussenbeleuchtung/<%=Url_tip%>" itemprop="item" title="<%=Titolo_1_tip%>"><span itemprop="name"><%=Titolo_1_tip%></span></a><meta itemprop="position" content="3" /></li>
								<li itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a href="/lichtmarkenlampen-innenbeleuchtung-aussenbeleuchtung/<%=Url_cat%>" itemprop="item" title="<%=Titolo_1_cat%>"><span itemprop="name"><%=Titolo_1_cat%></span></a><meta itemprop="position" content="4" /></li>
            ...
            <div class="col-md-4">
                <div class="panel panel-default" style="box-shadow: 0 3px 5px #ccc; position: relative;" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
					<p class="btn-add" style="position: absolute; right: 15px; top: 15px; font-size: 1.5em; z-index: 5">
						<a href="/de/preferiti.asp?id=<%=id%>" rel="nofollow" data-toggle="tooltip" data-placement="top" title="Zur Wunschliste hinzufugen"><i class="fa fa-heart"></i></a>
					</p>
					<%if prezzoarticolo=0 then%>
						...
						<ul class="list-group text-center">
							...
							<li class="list-group-item" style="padding-top: 20px">
	                            <p>
									<span class="price-old">Listenpreis  <b><%=prezzolistino%> &euro;</b><meta itemprop="priceCurrency" content="EUR" /></span><br /><br />
									Wollen Sie den Preis von Cristalensi wissen?
	                            </p>
	                        </li>
							<li class="list-group-item" style="padding-top: 20px; background-color:#f5f5f5;">
								<%if offerta=10 then%>
									DAS PRODUKT IST NICHT VERFUGBAR
								<%else%>
									<%if Len(Consegna)>0 then%>
									<small><em><%=Consegna%></em></small>
									<%else%>
									<small><em>Versand innerhalb von 30 Tagen</em></small>
									<%end if%>
								<%end if%>
							<li>
						</ul>
	                    <div class="panel-footer">
	                        <a data-fancybox data-src="#hidden-content-prev" href="javascript:;" rel="nofollow" class="btn launch-prev btn-danger btn-block" title="Fordern Sie ein Angebot bei unseren Mitarbeitern an">Fordern Sie ein Angebot bei unseren Mitarbeitern an <i class="glyphicon glyphicon-shopping-cart"></i></a>
	                    </div>
							<%else%>
								<%if offerta=10 then%>
									<ul class="list-group text-center">
									<li class="list-group-item" style="padding-top: 20px">
											<p>
													<span class="price-new">Das Produkt ist nicht verfugbar</span><br />
											</p>
									</li>
									</ul>
								<%else%>
									...
									<ul class="list-group text-center" style="margin-bottom: 0;">
															...
															<%if prezzoarticolo<>0 then%>
																<%if idsession=0 and prezzoprodottosoloclienti="si" then%>
																	...
																	<%if prezzolistino<>0 then%>
																		<span class="price-old">Listenpreis <b><%=prezzolistino%> &euro;</b></span>
																	<%end if%>
																<%Else%>
																	<%if prezzolistino<>0 then%>
																		<span class="price-old">Preis: <b><del>&nbsp;<%=prezzolistino%> &euro;&nbsp;</del></b></span><br>
																	<%end if%>
																	<span class="price-new"><span itemprop="price"><%=prezzoarticolo%></span> &euro;<meta itemprop="priceCurrency" content="EUR" /></span><br><small>Inkl. MwSt</small>
											...
											<%else%>
												...
												<%else%>
													<li class="list-group-item" style="background-color:#f5f5f5;">
														<small><em><i class="fa fa-truck"></i> Versand innerhalb von 30 Tagen</em></small>
													</li>
												<%end if%>
											<%end if%>
											<%if idsession=0 and prezzoprodottosoloclienti="si" then%>
												<%'qui eventualmente possiamo metter un banner per rafforzare iscrizione e sconto%>
												<li class="list-group-item" style="background-color: #FF972C; Color: #fff">Preis mit EXTRA DISCOUNT<br />für alle registrierten Kunden vorbehalten</li>
											<%else%>
												<%
												...
												<%if col_rs.recordcount>0 then%>
												<li class="list-group-item">
														<select name="colore" id="colore" class="selectpicker show-menu-arrow  show-tick" data-size="4" title="Wahle die Farbe">
														<%
												...
												<%if lam_rs.recordcount>0 then%>
												<li class="list-group-item">
														<select name="lampadina" id="lampadina" class="selectpicker show-menu-arrow  show-tick" data-size="4" title="Wahlen Sie die Gluhbirne oder das Glas">
														<%
												...
												%>
												<li class="list-group-item">
														<input type="number" data-width="auto" class="form-control" name="quantita" id="quantita" placeholder="Wie viele Stucke?" aria-label="Pezzi">
												</li>
											<%end if%>
									...
									<div class="panel-footer">
											<%if idsession=0 and prezzoprodottosoloclienti="si" then%>
												<a href="/de/iscrizione.asp?prov=3" id="invia_qta_2" rel="nofollow" class="btn btn-danger btn-block" title="Melden Sie sich an, um die Rabatte zu sehen!"><i class="glyphicon glyphicon-log-in"></i>&nbsp;&nbsp;Anmelden oder registrieren!</a>
											<%else%>
												<a href="#" onClick="return verifica_1();" id="invia_qta_2" rel="nofollow" class="btn btn-danger btn-block" style="padding: 10px 0px;" title="IN DEN WARENKORB AUFNEHMEN <%=titolo_prodotto%>&nbsp;<%=codicearticolo%>">IN DEN WARENKORB AUFNEHMEN <i class="glyphicon glyphicon-shopping-cart"></i></a>
											<%end if%>
									</div>
									</form>
								<%end if%>
				...
				<div class="panel panel-default hidden visible-sm-block visible-xs-block user-comment" itemprop="review" itemscope itemtype="http://schema.org/Review">
					...
					<ul class="list-group">
						...
						<li class="list-group-item"><i class="fa fa-user"></i> <em><span itemprop="description"><%=Left(NoHTML(com_rs("Testo")), 100)%>...</span><span itemprop="author" style="display: none;"><%=NomeIscritto%></span>
							<span itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">Bewertung: <meta itemprop="worstRating" content = "1"><span itemprop="ratingValue"><%=com_rs("Valutazione")%></span>/<span itemprop="bestRating">5</span></span></em></li>
						<%
							com_rs.movenext
							loop
							%>
