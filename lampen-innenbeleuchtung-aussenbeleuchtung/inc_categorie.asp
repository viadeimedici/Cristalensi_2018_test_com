...
if cr_rs.recordcount>0 then
  Titolo_1=cr_rs("Titolo_1_de")
  Titolo_2=cr_rs("Titolo_2_de")
  Url=cr_rs("Url_de")
  Title=cr_rs("Title_de")
  Description=cr_rs("Description_de")
  Descrizione=cr_rs("Descrizione_de")
  Img=cr_rs("Img")
...
<head>
    ...
    <meta name="description" content="<%if Len(Description)>0 then%><%=Description%><%else%>Durchsuchen Sie den Online-Katalog von <%=Titolo_1%>: von Cristalensi Rabatte auf alle zum Verkauf stehenden Beleuchtungsprodukte.<%end if%>">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta property="og:description" content="<%if Len(Description)>0 then%><%=Description%><%else%>Durchsuchen Sie den Online-Katalog von <%=Titolo_1%>: von Cristalensi Rabatte auf alle zum Verkauf stehenden Beleuchtungsprodukte.<%end if%>">
    ...
    function CambiaPagina()
    {
       ...
       document.modulopaginazione.action = "/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&order=<%=order%>#inizioelenco";
    ...
    <div class="container content">
        ...
        <div class="col-xl-12">
            <ol class="breadcrumb" itemtype="http://schema.org/BreadcrumbList">
                ...
                <li itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=Url_gr%>" itemprop="item" title="<%=Titolo_1_gr%>"><span itemprop="name"><%=Titolo_1_gr%></span></a><meta itemprop="position" content="2" /></li>
                <li itemprop="itemListElement" itemtype="http://schema.org/ListItem"><a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=Url_tip%>" itemprop="item" title="<%=Titolo_1_tip%>"><span itemprop="name"><%=Titolo_1_tip%></span></a><meta itemprop="position" content="3" /></li>
            ...
            <div class=" top-buffer">
                <nav class="navbar hidden-xs">
                    ...
                    <ul class="list-unstyled side-list">
                        ...
                        %>
                        <li><a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=Url%>" title="<%=Titolo_1%>" <%if pkid=pkid_categoria then%>class="active"<%end if%>><%=Titolo_1%></a></li>
                        <%
                        tip_rs.movenext
                        loop
                        %>
                        ...
                        <div class="container-fluid">
                            ...
                            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                                ...
                                <ul class="nav navbar-nav">
                                    <li <%if order=3 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?order=3"><i class="glyphicon glyphicon-eur"></i> - </a></li>
                                    <li <%if order=4 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?order=4"><i class="glyphicon glyphicon-eur"></i> + </a></li>
                                </ul>
                                <p class="navbar-text">alphabetisch</p>
                                <ul class="nav navbar-nav">
                                    <li <%if order=1 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?order=1">A/Z</a></li>
                                    <li <%if order=2 then%>class="active"<%end if%>><a style="border: solid 1px #e7e7e7; border-top: none; border-bottom: none;" href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?order=2">Z/A</a></li>
                                </ul>
                            </div>
                        </div>
                    </nav>
                      ...
                      <div class="info">
                          <div class="row">
                              <div class="price-details col-md-6">
                                  ...
                                  <p class="details"><span>code: <b title="<%=codicearticolo%>"><%=codicearticolo%></b></span> <span>marke: <b><a href="<%=url_produttore%>" title="<%=produttore%>"><%=produttore%></a></b></span></p>
                ...
                <div class="col-lg-12">
                    <nav aria-label="Page navigation center-block">
                        <ul class="pagination">
                            ...
                            <%if p > 2 then%>
                            <li><a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?p=1&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&order=<%=order%>#inizioelenco">Erste Seite</a></li>
                            <%end if%>
                            <% if p > 1 then %>
                            <li>
                                <a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?p=<%=p-1%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&order=<%=order%>#inizioelenco" aria-label="Previous">
                            ...
                            <li><a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?p=<%=Page%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&order=<%=order%>#inizioelenco"><%=page%></a></li>
                            <%end if%>
                            ...
                            <li>
                                <a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?p=<%=p+1%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&order=<%=order%>#inizioelenco" aria-label="Next">
                                    <span aria-hidden="true">&raquo;</span>
                                </a>
                            </li>
                            <%end if%>
                            <%if maxPage>5 and cInt(p)<>prod_rs.PageCount then%>
                            <li><a href="/lampen-innenbeleuchtung-aussenbeleuchtung/<%=toUrl%>?p=<%=prod_rs.PageCount%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&order=<%=order%>#inizioelenco">Letzte Seite</a></li>
                            <%end if%>
